...
private def runProtocol (p : Nat) [Fact (Nat.Prime p)] (n : Nat)
    ...
    : List UInt64 × List (UInt64 × UInt64) :=
  ...
  let claims := (List.finRange (n + 1)).map
    fun i => UInt64.ofNat (transcript.claims initialClaim i).val
  let roundPolys := (List.finRange n).map fun i =>
    let poly := transcript.roundPolys i
    let v0 := UInt64.ofNat (CPoly.CMvPolynomial.eval (fun _ => (0 : ZMod p)) poly).val
    let v1 := UInt64.ofNat (CPoly.CMvPolynomial.eval (fun _ => (1 : ZMod p)) poly).val
    (v0, v1)
  (claims, roundPolys)
